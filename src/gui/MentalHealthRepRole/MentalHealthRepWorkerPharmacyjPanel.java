...
public class MentalHealthRepWorkerPharmacyjPanel extends javax.swing.JPanel {
    ...
    public MentalHealthRepWorkerPharmacyjPanel(JPanel logincontainerJPanel,JPanel container, Users user, HealthNGO system) {
        initComponents();
        this.system=system;
    ...
    private void initComponents() {
        ...
        previousRequestsjTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Request", "Sender", "Status", "Requested", "Note", "Prescription", "Result"
            }
        ));
...
//        System.out.println(requests);
        if(requests.contains("Controller.WorkQueue.PrescriptionNotesWorker")){
            PrescriptionNotesWorker request = (PrescriptionNotesWorker)previousRequestsjTable.getValueAt(selectedRow, 0);
            MentalHealthRepWorkerPrescriptionRequestjPanel mhwsuicideIdeationProcess=new MentalHealthRepWorkerPrescriptionRequestjPanel(logincontainerJPanel,container, user ,system,request);
            CardLayout layout=(CardLayout)container.getLayout();
    ...
    public void displayPrescriptions(){
        ...
        for (WorkRequest order : user.getWorkQueue().getWorkRequestList()){
            ...
            if(request.contains("Controller.WorkQueue.PrescriptionNotesWorker")){
                row[0] = order;
                row[1] = (order.getSender()!=null)?order.getSender():"";
                row[2] = order.getStatus();
                row[3] = order.getRequestDate();
                String note = ((PrescriptionNotesWorker)order).getNotes();
                row[4] = note == null ? "None" : note;
                String msg = ((PrescriptionNotesWorker)order).getMessage();
                row[5] = msg == null ? "None" : msg;
                String result = ((PrescriptionNotesWorker)order).getRequesteResult();
